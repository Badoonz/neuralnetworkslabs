function [weights, iterations] = error_correction(inputs, d, starting_weights, max_iterations)
  transposed_inputs = inputs.';

  if isempty(starting_weights)
  ...
  while 1
    # step 1
    z = weights(1);
    inputs_row = transposed_inputs(j, :);
    ...
    if y != d(j)
      ...
      for k=1:size(inputs_row)(2)
        weights(k+1) = weights(k+1) + delta * inputs_row(k);
      endfor
    endif
